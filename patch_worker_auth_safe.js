...
let content = fs.readFileSync('C:/Users/김진곤/Downloads/alba-gon/client-backup/src/pages/WorkerApp.tsx', 'utf8');
let file = 'C:/Users/김진곤/Downloads/alba-gon/client/src/pages/WorkerApp.tsx';

// 1. targetStock logic patch
content = content.replace('targetStock: 10,', 'targetStock: detectedProduct ? detectedProduct.targetStock : 1,');

// 2. add QR URL parser
const qrRegex = /const handleBarcodeDetected = \(barcode: string\) => \{\s*if \(step !== 'IDLE'\) return;\s*setActiveBarcode\(barcode\);/;
const qrReplace = `const handleBarcodeDetected = (rawBarcode: string) => {
    if (step !== 'IDLE') return;
...
content = content.replace(qrRegex, qrReplace);
...
// 6. Update handleSaveQuantity to use authName
...
content = content.replace('  return (\n    <div className="max-w-md mx-auto h-screen', loginRender + '\n  return (\n    <div className="max-w-md mx-auto h-screen');
...
// 9. Fix __BUILD_TIME__
content = content.replace(/v\{__APP_VERSION__\} \(최신버전\)/, '최종 업데이트: {__BUILD_TIME__}');

fs.writeFileSync(file, content, 'utf8');
console.log('Restored and patched WorkerApp!');
