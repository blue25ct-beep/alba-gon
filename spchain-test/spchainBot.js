...
async function processSpchainOrder(orderPayload) {
...
console.log('[SPChain Bot] MQTT 연결 중...');
const client = mqtt.connect('wss://broker.emqx.io:8084/mqtt');

client.on('connect', () => {
    console.log(`[SPChain Bot] 연결 성공! Topic: ${TOPIC_REQ}`);
...
client.on('message', async (topic, message) => {
    try {
        ...
        if (payload.items) {
            console.log('[SPChain Bot] 새 주류 주문 수신:', payload);
            const result = await processSpchainOrder(payload);
            const reply = {
                type: 'ORDER_RESULT',
            ...
            client.publish(TOPIC_RES, JSON.stringify(reply));
            console.log('[SPChain Bot] 처리 결과 회신 완료');
        }
    } catch (e) {
        console.error('[SPChain Bot] 메시지 처리 오류:', e);
    }
});
