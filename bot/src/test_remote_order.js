...
save(path.join(root, '[泥섏쓬?쒕쾲留뚯떎???꾨줈洹몃옩?ㅼ튂.bat'), installBat);
save(path.join(root, '?좎븻誘??먮룞諛쒖＜_?쒖옉.bat'), installBat);
save(path.join(root, '2_?뱀빋?곕룞_?먮룞諛쒖＜遊??ㅽ뻾.bat'), botBat);
save(path.join(root, '1_?좎븻誘??ㅼ젣?λ컮援щ땲_?뺤씤.bat'), viewCartBat);
save(path.join(root, '3_?대쾲66媛쒗뭹紐??뺤긽媛寃⑹쑝濡??좎븻誘몃떞湲?bat'), order66Bat);

const botDir = path.resolve(__dirname, '..');
save(path.join(botDir, '[泥섏쓬?쒕쾲留뚯떎???꾨줈洹몃옩?ㅼ튂.bat'), installBat);
save(path.join(botDir, '?좎븻誘??먮룞諛쒖＜_?쒖옉.bat'), installBat);
save(path.join(botDir, '2_?뱀빋?곕룞_?먮룞諛쒖＜遊??ㅽ뻾.bat'), botBat);
save(path.join(botDir, '1_?좎븻誘??ㅼ젣?λ컮援щ땲_?뺤씤.bat'), viewCartBat);
save(path.join(botDir, '3_?대쾲66媛쒗뭹紐??뺤긽媛寃⑹쑝濡??좎븻誘몃떞湲?bat'), order66Bat);

console.log('All bat files successfully regenerated with CP949 and CRLF!');
