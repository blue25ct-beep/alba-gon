...
sequenceDiagram
    ...
    participant 클라우드 as ☁️ 클라우드 전용 우체통 (albagom/orders/store_047458)
    participant 매장PC as 💻 매장 카운터 PC (발주 봇)
    participant 유앤미 as 🏢 유앤미24 본사 (younme24.com)

    Note over 매장PC,클라우드: 매장 PC가 켜지면 우체통에 자동 접속 ("047458번 매장 대기 중")
...
2. **가맹점 번호(`047458`)가 고유 우체통 주소입니다:** 클라우드 서버에 사장님 매장 번호표로 된 `store_047458` 우체통이 상시 열려 있습니다.
3. **PC를 바꿔도 즉시 자동 인식:** 
   - 새 컴퓨터에서 발주 봇(`2_웹앱연동_자동발주봇_실행.bat`)을 켜는 순간, 새 컴퓨터가 `store_047458` 우체통으로 출근 도장을 찍고 기다립니다.
